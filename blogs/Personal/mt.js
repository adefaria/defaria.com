...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://defaria.com/mt/mt-comments.cgi?__mode=userinfo&blog_id=5&jsonp=' + cb + '&ts=' + ts + '&sid=' + u.sid;
        (document.getElementsByTagName('head'))[0].appendChild(script);
    }
}
...
function mtVerifySession(cb) {
    ...
    var ts = new Date().getTime();
    var u = mtGetUser();
    script.src = 'http://defaria.com/mt/mt-comments.cgi?__mode=verify_session&blog_id=5&jsonp=' + cb + '&ts=' + ts + '&sid=' + u.sid;
    (document.getElementsByTagName('head'))[0].appendChild(script);
}
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '6ef824e0a60d8045b3b3db18acc7d04301da997f';
...
function mtSignIn() {
    ...
    var url = 'http://defaria.com/mt/mt-comments.cgi?__mode=login&blog_id=5';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fdefaria.com%2Fblogs%2FPersonal%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://defaria.com/mt/mt-comments.cgi?__mode=handle_sign_in&static=0&logout=1&blog_id=5';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
